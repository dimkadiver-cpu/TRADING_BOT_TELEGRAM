"""Parser Event Envelope v1.

Minimal parser-side contract used as the central bridge between legacy
``TraderParseResult`` output and the canonical v1 normalizer.
"""
...
class UpdateOperationRaw(EnvelopeBaseModel):
    ...
    def _validate_op(self) -> "UpdateOperationRaw":
        ...
        populated = [name for name in expected_map.values() if getattr(self, name) is not None]
