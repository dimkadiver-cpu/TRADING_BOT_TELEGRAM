...
class TraderBProfileParser:
    ...
    def _classify_message(self, *, prepared: dict[str, Any]) -> str:
        ...
        if self._contains_any(normalized, self._as_markers("ignore_markers")):
            return "INFO_ONLY"

        has_symbol = _extract_symbol(raw_text) is not None
        has_side = _extract_side(normalized) is not None
        ...
        if has_symbol and has_side and has_entry and has_stop and has_tp:
            return "NEW_SIGNAL"
        if has_symbol and has_side and has_entry and has_stop and self._contains_any(normalized, ("тейк", "tp", "target", "тп")):
            return "SETUP_INCOMPLETE"

        update_markers = self._as_markers("classification_markers", "update")
        if self._contains_any(normalized, update_markers) or self._contains_any(
            normalized,
            (
                "закрыт",
                "закрываю",
    ...
    def _extract_intents(self, *, prepared: dict[str, Any], message_type: str) -> list[str]:
        ...
        if self._contains_any(normalized, self._as_markers("intent_markers", "U_CLOSE_FULL")) or self._contains_any(
            normalized,
            ("сделка полностью закрыта", "закрыта", "закрываю", "закрыть позицию"),
        ):
        ...
        if self._contains_any(normalized, ("тейк достиг", "take profit hit", "tp hit", "target hit")):
    ...
    def _extract_entities(self, *, prepared: dict[str, Any], intents: list[str], message_type: str) -> dict[str, Any]:
        ...
        if message_type in {"NEW_SIGNAL", "SETUP_INCOMPLETE"}:
            ...
            order_type = "MARKET" if "по текущим" in normalized else "LIMIT"
            entities.update(
                {
                    ...
                    "market_context": "SPOT" if "сделка на споте" in normalized else None,
...
def _extract_side(normalized: str) -> str | None:
    if any(marker in normalized for marker in ("лонг", "long", "buy")):
        return "LONG"
    if any(marker in normalized for marker in ("шорт", "short", "sell")):
        return "SHORT"
    return None
