...
class TraderBProfileRealCasesTests(unittest.TestCase):
    ...
    def test_stop_hit_variant_is_info_only(self) -> None:
        text = "Очень обидный стоп, идея в целом отработала"
        result = self.parser.parse_message(text, _context(text=text, reply_to=507))
        self.assertEqual(result.message_type, "INFO_ONLY")
        self.assertEqual(result.intents, [])

    def test_passive_close_in_be_is_info_only(self) -> None:
        text = "Закрыта в БУ"
        result = self.parser.parse_message(text, _context(text=text, reply_to=510))
        self.assertEqual(result.message_type, "INFO_ONLY")
        self.assertEqual(result.intents, [])

    def test_passive_close_with_stop_loss_is_info_only(self) -> None:
        text = "Сделка закрыта по стоп лоссу -1%"
        result = self.parser.parse_message(text, _context(text=text, reply_to=511))
        self.assertEqual(result.message_type, "INFO_ONLY")
        self.assertEqual(result.intents, [])
    ...
    def test_passive_obidny_stop_is_info_only(self) -> None:
        text = "Очень обидный стоп, идея в целом отработала"
        result = self.parser.parse_message(text, _context(text=text, reply_to=512))
        self.assertEqual(result.message_type, "INFO_ONLY")
        self.assertEqual(result.intents, [])

    def test_passive_stop_loss_is_info_only(self) -> None:
        text = "К сожалению стоп лосс, рынок прям медвежий"
        result = self.parser.parse_message(text, _context(text=text, reply_to=513))
        self.assertEqual(result.message_type, "INFO_ONLY")
        self.assertEqual(result.intents, [])
    ...
    def test_move_stop_to_be_from_bu_only(self) -> None:
        text = "БУ"
        result = self.parser.parse_message(text, _context(text=text, reply_to=509))
        self.assertEqual(result.message_type, "UPDATE")
        self.assertIn("U_MOVE_STOP_TO_BE", result.intents)
