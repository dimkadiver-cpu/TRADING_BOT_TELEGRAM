...
_LIMIT_ENTRY_RE = re.compile(
    r"вход\s+(?:(?:с|по)\s+)?лимит(?:кой|ку|ка|ке|ки)?\s*(?P<value>\d+(?:[.,]\d+)?)?",
    re.IGNORECASE,
)
...
class TraderCProfileParser(TraderBProfileParser):
    ...
    def _extract_intents(self, *, prepared: dict[str, Any], message_type: str, entities: dict[str, Any]) -> list[str]:
        ...
        if self._contains_any(normalized, _CLOSE_FULL_MARKERS) or re.search(r"\bзакрыл\w*\s+\d", normalized):
        ...
        if self._contains_any(normalized, _UPDATE_STOP_MARKERS) or _STOP_AT_PRICE_RE.search(raw_text):
    ...
    def _is_operational_update(*, normalized: str, raw_text: str = "") -> bool:
        if any(
            ...
            return True
        if "tp" in normalized or "тп" in normalized:
            return True
...
def _extract_take_profits(raw_text: str) -> list[float]:
    ...
    for match in _TP_COMPACT_RE.finditer(raw_text):
        ...
        if value is not None and value > 9 and value not in out:   # guard: skip RR ratios
            out.append(value)
    if out:
        return out
    ...
    if header:
        ...
        for match in _TP_LIST_LINE_RE.finditer(block):
            value = _to_float(match.group("value"))
            # Skip small integers that are RR ratios (1-9) or list indices
            if value is not None and value > 9 and value not in out:
                out.append(value)
