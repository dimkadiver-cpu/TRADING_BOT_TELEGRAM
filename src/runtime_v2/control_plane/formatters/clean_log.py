...
def _append_signal_body(lines: list[str], p: dict) -> None:
    ...
    if p.get("leverage") is not None:
        lines.append(f"Leverage: x{p['leverage']}")
    if p.get("parse_status") == "PARTIAL":
        warnings = p.get("parse_warnings") or []
        warn_str = ", ".join(warnings) if warnings else "incomplete parse"
        lines.append(f"Parser: PARTIAL ({warn_str})")


def _signal_notification(p: dict, *, emoji: str, event_label: str, source: str) -> str:
    lines = _header(emoji, p.get("chain_id"), event_label, p.get("symbol"), p.get("side"))
    _append_signal_body(lines, p)
    if p.get("trader_id") or p.get("account_id"):
        lines.append(_SEP)
        if p.get("trader_id"):
            lines.append(f"Trader: {p['trader_id']}")
        if p.get("account_id"):
            lines.append(f"Exchange Account: {p['account_id']}")
    if not lines or lines[-1] != _SEP:
        lines.append(_SEP)
    if p.get("reason"):
        lines.append(f"Rejected: {p['reason']}")
    lines.append(f"Source: {source}")
    if p.get("link"):
        lines.extend([_SEP, p["link"]])
...
def _final_result_lines(final_result: dict | None) -> list[str]:
    ...
    lines.append(f"ROI net: {_fmt_pct(result.get('roi_net_pct'), signed=True)}")
    lines.append(f"Total PnL net: {_fmt_money(result.get('total_pnl_net'), signed=True)}")
    lines.append(f"Gross PnL: {_fmt_money(result.get('gross_pnl'), signed=True)}")
    lines.append(f"Fees: {_fmt_money(result.get('fees'), signed=True)}")
    lines.append(f"Funding: {_fmt_money(result.get('funding'), signed=True)}")
    return lines


def _close_metric_lines(p: dict, *, price_label: str, price_value) -> list[str]:
    fee_rate = p.get("fee_rate") if "fee_rate" in p else None
    return [
        f"{price_label}: {_num(price_value) if price_value is not None else 'n/a'}",
        f"Qty: {_num(p.get('closed_qty')) if p.get('closed_qty') is not None else 'n/a'}",
        f"PnL: {_fmt_money(p.get('pnl'), signed=True)}",
        f"Fee: {_fmt_money(p.get('fee'))}",
        f"Fee rate: {float(fee_rate) * 100:.3f}%" if fee_rate is not None else "Fee rate: n/a",
    ]
...
def _signal_accepted(p: dict) -> str:
    return _signal_notification(
        p,
        emoji="\u2705",
        event_label="SIGNAL ACCEPTED",
        source=p.get("source", "trader_signal"),
    )
...
def _tp_filled(p: dict, final: bool) -> str:
    ...
    if final:
        return _closed_template(
            {
                **p,
                "close_reason": "FINAL TP FILLED",
            },
            emoji=emoji,
            price_label=f"TP_{level}" if level is not None else "TP",
            price_value=display_price,
        )
    lines.append("")
    lines += _footer(p.get("source", "exchange"))
    return _finalize(lines)
...
def _sl_filled(p: dict) -> str:
    return _closed_template(
        ...
        emoji="\U0001f6d1",
        price_label="SL",
        price_value=p.get("sl_price", p.get("fill_price")),
    )
...
def _be_exit(p: dict) -> str:
    ...
    price_value = p.get("sl_price") if p.get("sl_price") is not None else p.get("exit_price", p.get("fill_price"))
    return _closed_template(
        p,
        emoji="\u26a1",
