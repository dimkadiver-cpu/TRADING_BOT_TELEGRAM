...
def format_trades(view: TradesView) -> str:
    ...
    if not view.rows:
        ...
        sym_w = max((len(r.symbol) for r in view.rows), default=6)
        ...
        for r in view.rows:
            lines.append(
                f"{str(r.chain_id).ljust(id_w)} | {r.symbol.ljust(sym_w)} | {r.side.ljust(side_w)} | {r.state.ljust(state_w)} | {_protection(r)}"
            )
