...
Canonical Model v1 uses:

- `entry_structure` at signal level
- `entry_type` at entry-leg level

So:

- there is **no global `entry_type`**
- the true order semantics live inside `entries[*]`
- `entry_structure` describes the overall plan shape

### Canonical entry structures
...
Legacy values are converted as follows:

- `SINGLE` -> `ONE_SHOT`
- `ZONE` -> `RANGE` with `LIMIT` legs
- `AVERAGING` -> `TWO_STEP` or `LADDER` with `LIMIT` legs
- legacy `entry_plan_type` -> canonical `entry_structure` + per-leg `entry_type`

### Notes

Mixed plans are naturally supported.
...
- leg 1: `entry_type = MARKET`
- leg 2: `entry_type = LIMIT`
...
1. `primary_class = SIGNAL`
   - `signal` is required
   - `update` must be absent
   - `report` is optional only in exceptional composite cases

2. `primary_class = UPDATE`
   - `update` is required
   - `report` is optional
   - `signal` must be absent
...
4. `primary_class = INFO`
   - `signal`, `update`, `report` must all be absent

### SIGNAL validation

A `SIGNAL` is `PARSED` when at least these are present:

- `symbol`
- `side`
- `entry_structure`
- `stop_loss`
- at least one `take_profit`

Entry structure cardinality rules:

- `ONE_SHOT` -> at least 1 entry leg
- `TWO_STEP` -> at least 2 entry legs
- `RANGE` -> exactly 2 entry legs
- `LADDER` -> at least 3 entry legs

Exception:

- `ONE_SHOT` with `entry_type = MARKET` may have `price = null`

A `SIGNAL` is `PARTIAL` when it is clearly a signal but one or more required signal fields are missing or the entry structure is inconsistent.

### UPDATE validation

An `UPDATE` is `PARSED` when:

- `operations` contains at least one operation
- every operation has its minimum required fields

Minimum fields by operation:

- `SET_STOP` -> `stop_target`
- `CLOSE` -> at least one of `close_fraction`, `close_scope`, `close_price`
- `CANCEL_PENDING` -> no extra mandatory field
- `MODIFY_ENTRIES` -> `mode` required and `entries` non-empty
- `MODIFY_TARGETS` -> `mode` required and `take_profits` non-empty

### REPORT validation

A `REPORT` is `PARSED` when:

- there is at least one `event`
  or
- `reported_result` is present

### INFO validation

`INFO` is used only when the message does not contain a structured signal, structured update, or structured report.
