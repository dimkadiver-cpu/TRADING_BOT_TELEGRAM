...
Separatamente dal `message_class`, il parser emette uno stato tecnico:
...
- `primary_class` è la classificazione business principale del messaggio
- `signal` è normalmente valorizzato quando `primary_class == SIGNAL`
- `update` è valorizzato quando il messaggio contiene una o più operazioni operative
- `report` è valorizzato quando il messaggio contiene uno o più eventi/report semantici
- `INFO` ha normalmente `signal = None`, `update = None`, `report = None`

### 5.2 Regola per messaggi compositi

Il modello v1 ammette messaggi compositi.

Esempio:
- "TP1 preso" → `report`
- "stop a breakeven" → `update`
- "cancella ordine residuo" → `update`

In un caso simile:
- `primary_class = UPDATE`
- `update` è valorizzato
- `report` è valorizzato

Regola pratica:
- se esiste almeno un'operazione operativa, `primary_class = UPDATE`
- se esistono solo eventi/outcome senza istruzioni operative, `primary_class = REPORT`
...
class UpdateOperation:
    ...
    set_signal_state: SetSignalStateOperation | None
...
### 10.6 Regola per report composito dentro un update

Un evento `REPORT` può coesistere nello stesso messaggio insieme a operazioni `UPDATE`.

Esempio:

> "Auguri primo TP +0,5%, stop a breakeven, altro ordine eliminiamo"

Interpretazione canonica:
- `report.events` contiene `TP_HIT` con risultato riportato `+0,5%`
- `update.operations` contiene `SET_STOP(target_type=ENTRY)`
- `update.operations` contiene `CANCEL_PENDING`
- `primary_class = UPDATE`

Quindi `REPORT payload` non è esclusivo: è un componente semantico che può convivere nello stesso envelope con `UPDATE payload`.
...
9. i messaggi compositi sono ammessi: `update` e `report` possono coesistere nello stesso envelope, con `primary_class` come classificazione principale.
...
5. regola finale di priorità per valorizzare `primary_class` nei messaggi compositi;
