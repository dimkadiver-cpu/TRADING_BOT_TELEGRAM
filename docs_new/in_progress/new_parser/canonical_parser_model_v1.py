...
class PriceValue(CanonicalBaseModel):
    raw: str
    value: float
...
class CanonicalMessage(CanonicalBaseModel):
    ...
    def _validate_top_level(self) -> "CanonicalMessage":
        if self.primary_class == "SIGNAL":
            ...
            # report optional in exceptional composite cases
        ...
        if self.parse_status == "PARSED":
            if self.primary_class == "SIGNAL":
                assert self.signal is not None  # guarded above
