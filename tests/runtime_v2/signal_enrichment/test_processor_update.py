...
def _make_processor(tmp_path, gate_mode: str = "block"):
    ...
    base = {
        ...
        "defaults": {
            "enabled": True, "gate_mode": gate_mode, "hedge_mode": False,
            "signal_policy": {
...
def test_warn_mode_produces_review(tmp_path):
    proc = _make_processor(tmp_path, gate_mode="warn")
    result = _make_update_result(source_intent="MOVE_STOP_TO_BE", canonical_message_id=3)
    enriched = proc.process(result)
